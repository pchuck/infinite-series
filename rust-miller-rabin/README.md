...
1. **Parallel Base Testing**: Miller-Rabin bases are distributed across threads when `-p` flag is used. Each thread tests a subset of the deterministic bases independently.

2. **Batch Processing**: When testing number ranges, the range is divided into chunks and processed concurrently by multiple threads.
