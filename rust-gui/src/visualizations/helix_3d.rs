...
pub fn draw(app: &mut crate::app::NumberVisualizerApp, ui: &mut egui::Ui, rect: egui::Rect) {
    ...
    for (x, y, depth, is_highlighted) in &projected {
        ...
        let temp_config = VisualizerConfig {
            highlight_size: (app.config.highlight_size as f32 * depth_factor) as usize,
            non_highlight_size: (app.config.non_highlight_size as f32 * depth_factor) as usize,
            highlight_color: adjust_brightness(app.config.highlight_color, depth_factor),
            non_highlight_color: adjust_brightness(app.config.non_highlight_color, depth_factor),
            ..app.config.clone()
        };

        if *is_highlighted {
            let size = temp_config.highlight_size as f32 / 2.0;
            painter.circle_filled(
                egui::Pos2::new(screen_x, screen_y),
                size.max(0.5),
                temp_config.highlight_color,
            );
        } else if temp_config.non_highlight_size > 0 {
            let size = temp_config.non_highlight_size as f32 / 2.0;
            painter.circle_filled(
                egui::Pos2::new(screen_x, screen_y),
                size.max(0.5),
                temp_config.non_highlight_color,
            );
        }
    }
}
